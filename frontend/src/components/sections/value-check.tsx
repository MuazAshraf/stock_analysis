  ...
  switch (verdict) {
    ...
    case "overvalued":
      return {
        label: "Overvalued",
        color: "#ef4444",
        // Light/dark variants — bg-red-50 alone made the banner stay pale-pink
        // in dark mode, where cream brand-fg text became invisible.
      ...
      <CardContent className="space-y-5">
        ...
        <div className={`p-5 rounded-xl ${cfg.bg} border ${cfg.border}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium text-brand-fg/60">Verdict</p>
              <p className="text-2xl font-bold" style={{ color: cfg.color }}>
                {cfg.label}
              </p>
            </div>
          </div>
          <p className="text-sm text-brand-fg/80 mt-3 leading-relaxed">
            {data.explanation}
          </p>
          <p className="text-xs text-brand-fg/50 mt-2 italic">{cfg.urdu}</p>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-4 rounded-xl bg-brand-bg">
                <p className="text-xs text-brand-fg/60">Intrinsic Value</p>
                <p className="text-lg font-bold text-brand-fg mt-1">
                  {fmtPKR(data.intrinsic_value)}
                </p>
                <p className="text-[10px] text-brand-fg/50 mt-1">per share</p>
              </div>
              <div className="p-4 rounded-xl bg-brand-bg">
                <p className="text-xs text-brand-fg/60">Current Price</p>
                <p className="text-lg font-bold text-brand-fg mt-1">
                  {fmtPKR(data.current_price)}
                </p>
                <p className="text-[10px] text-brand-fg/50 mt-1">market price</p>
              </div>
              <div className="p-4 rounded-xl bg-brand-bg">
                <p className="text-xs text-brand-fg/60">Margin of Safety</p>
                <p
                ...
                <p className="text-[10px] text-brand-fg/50 mt-1">
            ...
            {margin != null && (
              <div className="p-4 rounded-xl bg-brand-soft">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-medium text-brand-fg/60">
            ...
            <div className="p-3 rounded-lg bg-brand-bg border border-brand-border">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <span className="text-brand-fg/60">EPS:</span>{" "}
                  <span className="font-mono text-brand-fg">
                    {fmtPKR(data.eps_used)}
                  </span>
                </div>
                <div>
                  <span className="text-brand-fg/60">Book Value/Share:</span>{" "}
                  <span className="font-mono text-brand-fg">
                    {fmtPKR(data.book_value_used)}
                  </span>
                </div>
              </div>
              <p className="text-[10px] text-brand-fg/50 mt-2 leading-relaxed">
